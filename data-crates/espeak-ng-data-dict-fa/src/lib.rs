//! eSpeak NG dictionary data for `fa_dict`, embedded at compile time.
//!
//! ## Usage
//!
//! ```rust,no_run
...
//! espeak_ng_data_dict_fa::install(data_dir).unwrap();
//! ```

include!(concat!(env!("OUT_DIR"), "/files.rs"));

/// Install the embedded `fa_dict` into `dest_dir`.
