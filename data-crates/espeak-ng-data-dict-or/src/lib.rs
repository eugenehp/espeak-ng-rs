//! eSpeak NG dictionary data for `or_dict`, embedded at compile time.
//!
//! ## Usage
//!
//! ```rust,no_run
...
//! espeak_ng_data_dict_or::install(data_dir).unwrap();
//! ```

include!(concat!(env!("OUT_DIR"), "/files.rs"));

/// Install the embedded `or_dict` into `dest_dir`.
