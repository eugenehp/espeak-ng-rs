//! eSpeak NG dictionary data for `sr_dict`, embedded at compile time.
//!
//! ## Usage
//!
//! ```rust,no_run
...
//! espeak_ng_data_dict_sr::install(data_dir).unwrap();
//! ```

include!(concat!(env!("OUT_DIR"), "/files.rs"));

/// Install the embedded `sr_dict` into `dest_dir`.
