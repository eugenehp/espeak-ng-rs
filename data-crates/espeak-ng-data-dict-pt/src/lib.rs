//! eSpeak NG dictionary data for `pt_dict`, embedded at compile time.
//!
//! ## Usage
//!
//! ```rust,no_run
...
//! espeak_ng_data_dict_pt::install(data_dir).unwrap();
//! ```

include!(concat!(env!("OUT_DIR"), "/files.rs"));

/// Install the embedded `pt_dict` into `dest_dir`.
