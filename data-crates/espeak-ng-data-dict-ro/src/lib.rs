//! eSpeak NG dictionary data for `ro_dict`, embedded at compile time.
//!
//! ## Usage
//!
//! ```rust,no_run
...
//! espeak_ng_data_dict_ro::install(data_dir).unwrap();
//! ```

include!(concat!(env!("OUT_DIR"), "/files.rs"));

/// Install the embedded `ro_dict` into `dest_dir`.
