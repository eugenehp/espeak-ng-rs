//! Russian language dictionary for eSpeak NG, embedded at compile time.
//!
//! The Russian dictionary (`ru_dict`) is the largest single language file
//! in eSpeak NG (~8 MB) so it is published as its own crate.
//!
//! ## Usage
//!
//! ```rust,no_run
...
//! espeak_ng_data_dict_ru::install(data_dir).unwrap();
//! ```

include!(concat!(env!("OUT_DIR"), "/files.rs"));

/// Install the embedded `ru_dict` into `dest_dir`.
